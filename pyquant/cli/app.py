...
# Keep library/runtime chatter out of the pretty CLI output by default.
...
def train(
    ...
    """Train a Temporal Fusion Transformer for SYMBOL."""
    settings = _build_settings(period, no_macro, no_sentiment, no_sectors)
    console.print(f"[bold cyan]Training TFT for {symbol.upper()}[/bold cyan]")
    with console.status("Fetching data and training..."):
        result = tft.train(symbol, settings, max_epochs=epochs, progress=True)

    table = Table(title=f"Training complete — {result.symbol}", show_header=False)
    ...
    console.print(table)
    console.print("[dim]Next: pyquant forecast " + result.symbol + "[/dim]")
...
def forecast(
    ...
    fc = generate_forecast(symbol, settings)
...
def explain(
    ...
    panel = build_panel(symbol.upper(), settings)
    att = attention_to_series(interp, panel.index)
    peak = att.idxmax()
    console.print(
...
def scan(
    ...
    for ticker in tickers:
        ...
        signal = "[green]BUY[/green]" if pct > 2 else "[red]SELL[/red]" if pct < -2 else "HOLD"
        table.add_row(
            ticker,
